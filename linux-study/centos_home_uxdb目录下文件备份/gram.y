 ...
 *
 * gram.y
 *	  UXSINODB BISON rules/actions
 *
 * Portions Copyright (c) 1996-2017, UXsinoDB Global Development Group
...
#include "catalog/ux_am.h"
#include "catalog/ux_trigger.h"
...
%type <node>	stmt schema_stmt
		...
		CreateFdwStmt CreateForeignServerStmt CreateForeignTableStmt CreateVerticalTableStmt
...
%type <node>	columnDef partitionColumnDef columnOptions
...
%type <list>	var_list
%type <str>		ColId ColLabel var_name type_function_name param_name PartitionName
 ...
 * the set of keywords.  PL/uxSQL depends on this so that it can share the
 * same lexer.  If you add/change tokens here, fix PL/uxSQL to match!
 *
 * DOT_DOT is unused in the core SQL grammar, and so will always provoke
 * parse errors.  It is needed by PL/uxSQL.
 */
...
%token <keyword> ABORT_P ABSOLUTE_P ACCESS ACTION ADD_P ADMIN AFTER
	...
	PARALLEL PARSER PARTIAL PARTITION PASSING PASSWORD PLACING PLANS PLCOLS POLICY
	...
	SUBSCRIPTION SUBSTRING SYMMETRIC SYSID SYSTEM_P SYSDATE
	...
	VERBOSE VERSION_P VERTICAL VIEW VIEWS VOLATILE

	WHEN WHERE WHITESPACE_P WINDOW WITH WITHIN WITHOUT WORK WRAPPER WRITE

	XML_P XMLATTRIBUTES XMLCONCAT XMLELEMENT XMLEXISTS XMLFOREST XMLNAMESPACES
	XMLPARSE XMLPI XMLROOT XMLSERIALIZE XMLTABLE XLS
...
stmtblock:	stmtmulti
			{
				ux_yyget_extra(yyscanner)->parsetree = $1;
			}
		;
			...
			| IDENT
				{
					...
					}	/* @zwjs add for #58164, increase the role state to determine permissions */
					else if (strcmp($1, "enabled") == 0)
						$$ = makeDefElem("state", (Node *)makeInteger(TRUE), @1);
					else if (strcmp($1, "disabled") == 0)
						$$ = makeDefElem("state", (Node *)makeInteger(FALSE), @1);
					else
						ereport(ERROR,
...
/*****************************************************************************
 *
 * Create a new Uxdb DBMS user (role with implied login ability)
 *
 *****************************************************************************/
...
/*****************************************************************************
 *
 * Create a uxsinodb group (role without login ability)
 *
 *****************************************************************************/
...
set_rest_more:	/* Generic SET syntaxes: */
			...
			/* Special syntaxes invented by UXsinoDB: */
...
alter_table_cmd:
			...
			/* [MAVERICK] ALTER TABLE <name> ENABLE CONSTRAINT ... */
			| ENABLE_P TableConstraint
				{
					AlterTableCmd *n = makeNode(AlterTableCmd);
			...
			/* [MAVERICK] ALTER TABLE <name> DISABLE CONSTRAINT <name> [RESTRICT|CASCADE] */
			| DISABLE_P CONSTRAINT name opt_drop_behavior
				{
					AlterTableCmd *n = makeNode(AlterTableCmd);
					n->subtype = AT_DropConstraint;
					n->name = $3;
...
copy_opt_item:
			...
			| PLCOLS
				{
					$$ = makeDefElem("plcols", (Node *)makeInteger(TRUE), @1);
				}
			| FREEZE
				{
			...
			| XLS
				{
					$$ = makeDefElem("format", (Node *)makeString("xls"), @1);
				}
			| XML_P
				{
					$$ = makeDefElem("format", (Node *)makeString("xml"), @1);
				}
			| HEADER_P
				{
...
TableElement:
			columnDef							{ $$ = $1; }
			/*ShiQing #59711 2019.11.1 add partition name*/
			| partitionColumnDef				{ $$ = $1; }
			/*ShiQing #59711 2019.11.1 add partition name*/
			| TableLikeClause					{ $$ = $1; }
			| TableConstraint					{ $$ = $1; }
		;
...
/*ShiQing #59711 2019.11.1 add partition name*/
partitionColumnDef:  ColId Typename PartitionName create_generic_options ColQualList
				{
					ColumnDef *n = makeNode(ColumnDef);
					n->colname = $1;
					n->typeName = $2;
...
/* DEFAULT NULL is already the default for Uxdb.
...
SeqOptElem: AS SimpleTypename
			...
			| SEQUENCE NAME_P any_name
				{
					/* not documented, only used by ux_dump */
...
/*ShiQing 2019.10.23 #59711 create vertical table*/
/*****************************************************************************
 *
 *		QUERY:
 *             CREATE VERTICAL TABLE relname
 *
 *****************************************************************************/

CreateVerticalTableStmt:
	CREATE VERTICAL TABLE qualified_name '(' OptTableElementList ')'
			OptInherit OptPartitionSpec OptWith OnCommitOption OptTableSpace
				{
					CreateVerticalTableStmt *n = makeNode(CreateVerticalTableStmt);
					$4->relpersistence = RELPERSISTENCE_PERMANENT;
					n->relation = $4;
					n->tableElts = $6;
					n->inhRelations = $8;
 ...
 * ux_proc.  We check this at the grammar stage so that we can return a list
...
/* Syntax with commas is SQL-spec, without commas is Uxdb historical */
transaction_mode_list:
			transaction_mode_item
					{ $$ = list_make1($1); }
...
/*
 *	Though the equals sign doesn't match other WITH options, ux_dump uses
			...
			| select_with_parens opt_alias_clause
				{
					...
					*/
					...
					if ($2 == NULL)
					{
						...
						initStringInfo(&buf);
						appendStringInfo(&buf, "__new_alias__%d", ++extral_alias_counts);
						a->aliasname = buf.data;
						n->alias = a;
					}
 ...
 *		 the generic case to handle regular type-extensible Uxdb syntax.
 *		- thomas 1997-10-10
 *
 *****************************************************************************/
...
func_expr_common_subexpr:
			COLLATION FOR '(' a_expr ')'
				{
					$$ = (Node *) makeFuncCall(SystemFuncName("ux_collation_for"),
											   list_make1($4),
											   @1);
				}
			| CURRENT_DATE
            ...
            | SYSDATE
                {
                    $$ = makeSQLValueFunction(SVFOP_SYSDATE, -1, @1);
                }
			...
			| a_expr substr_for
				{
					 ...
					 * to int4.  The possible matches in ux_proc are
 ...
 * This presently includes *all* Uxdb keywords.
 */
ColLabel:	IDENT									{ $$ = $1; }
			| unreserved_keyword					{ $$ = pstrdup($1); }
			| col_name_keyword						{ $$ = pstrdup($1); }
 ...
 * the Uxdb grammar should appear in exactly one of these lists.
 *
...
{
	return list_make2(makeString("ux_catalog"), makeString(name));
}
...
{
	return makeTypeNameFromNameList(list_make2(makeString("ux_catalog"),
											   makeString(name)));
}
 ...
 * As of Uxdb 7.0, that problem doesn't exist anymore because there
